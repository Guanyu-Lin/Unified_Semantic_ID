...
class SemanticDataset(Dataset):
    ...
    def __init__(self, args, item2attribute, data_type='initialize'):
        self.args = args
        # self.item2attribute = item2attribute
        item2attribute["0"] = [0,0,0]
        # import pdb
        # pdb.set_trace()
        item2attribute[str((max(list(map(int, item2attribute.keys())))) + 1)] = [0,0,0]

        tmp = dict(sorted(item2attribute.items(), key=lambda item:int(item[0])))
        self.item2attribute = []
        for k, v in tmp.items():
            self.item2attribute.append(v)
    ...
    def __getitem__(self, index):
        ...
        # items = self.user_seq[index]
        attributes = self.item2attribute[index]

        assert self.data_type in {"initialize"}

        semantic_ids = attributes[:]


     
        cur_tensors = (
            torch.tensor(semantic_ids, dtype=torch.long),  # user_id for testin
        )

        return cur_tensors

    def __len__(self):
        return len(self.item2attribute)