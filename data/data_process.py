...
def Amazon(dataset_name, rating_score):
    ...
    # id_map = {}
    index = 0
    data_flie = 'data/Amazon/reviews_' + dataset_name + '.json.gz'
    # latest Amazon
...
def Amazon_meta(dataset_name, data_maps):
    ...
    '''
    datas = {}
    meta_flie = 'data/Amazon/meta_' + dataset_name + '.json.gz'
...
def get_attribute_Amazon(meta_infos, datamaps, attribute_core):
    ...
    print(f'before delete, category num:{len(attributes)}')
    new_meta = {}
    new_category = {}
    new_brand = {}
    for iid, info in tqdm.tqdm(meta_infos.items()):
        ...
        new_category[iid] = []
        try:
            new_meta[iid].append(info['title'])
        except:
            pass
        try:
            new_meta[iid].append(str(info['price']))
        except:
            pass
        try:
            if attributes[info['brand']] >= attribute_core:
                new_meta[iid].append(info['brand'])
                new_brand[iid].append(info['brand'])
        except:
            pass
        for cates in info['categories']:
            for cate in cates[1:]:
    ...
    for iid, attributes in new_meta.items():
        ...
        #     attribute_text.append(attribute)
        for brand in new_brand[iid]:
            if brand not in brand2id:
                brand2id[brand] = brand_id
                id2brand[brand_id] = brand
                brand_id+=1
            items2brand[item_id].append(brand2id[brand])
        for cate in new_category[iid]:
            if cate not in cate2id:
                cate2id[cate] = cate_id
                id2cate[cate_id] = cate
                cate_id+=1
            cateid2num[cate2id[cate]] += 1
            items2cate[item_id].append(cate2id[cate])
        items2attributestext[item_id] = ";".join(attributes)
        cate_lens.append(len(items2cate[item_id]))
    
    print(f'before delete, category num:{len(cate2id)}')
    print(f'attributes len, Min:{np.min(cate_lens)}, Max:{np.max(cate_lens)}, Avg.:{np.mean(cate_lens):.4f}')
    # 更新datamap
    datamaps['cate2id'] = cate2id
    datamaps['id2cate'] = id2cate
    datamaps['cateid2num'] = cateid2num
    return len(cate2id), np.mean(cate_lens), datamaps, items2attributestext, items2brand, items2cate
...
def main(data_name, data_type='Amazon'):
    ...
    if data_type == 'Amazon':
        ...
        attribute_num, avg_attribute, datamaps, item2attributes_text, item2brand, item2cate = get_attribute_Amazon(meta_infos, data_maps, attribute_core)
    else:
    ...
    # -------------- Save Data ---------------
    data_file = 'data/'+ data_name + str(user_core) + str(item_core) + '.txt'
    # item2attributes_file = 'data/'+ data_name + '_item2attributes.json'
    item2attributes_text_file = 'data/'+ data_name + str(user_core) + str(item_core) + '_item2attributes_text.json'
    item2brand_file = 'data/'+ data_name + str(user_core) + str(item_core) + '_item2brand.json'
    item2cate_file = 'data/'+ data_name + str(user_core) + str(item_core) + '_item2cate.json'

    with open(data_file, 'w') as out:
        for user, items in user_items.items():
            out.write(user + ' ' + ' '.join(items) + '\n')
    json_str = json.dumps(item2attributes_text)
    json_str_brand = json.dumps(item2brand)
    json_str_cate = json.dumps(item2cate)
    # with open(item2attributes_file, 'w') as out:
    #     out.write(json_str)
    with open(item2attributes_text_file, 'w') as out:
        out.write(json_str)
    with open(item2brand_file, 'w') as out:
        out.write(json_str_brand)
    with open(item2cate_file, 'w') as out:
        out.write(json_str_cate)
