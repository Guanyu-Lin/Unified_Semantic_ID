...
def get_user_attribute_seq(data_file, item2attribute, codebook_size):
    lines = open(data_file).readlines()
    user_seq = []
    user_seq_attribute = []
    item_set = set()
    for line in lines:
        user, items = line.strip().split(' ', 1)
        items = items.split(' ')
        item_attributes = [tuple(item2attribute[item][:codebook_size]) for item in items]
        items = [int(item) for item in items]
        user_seq.append(items)
        user_seq_attribute.append(item_attributes)
        item_set = item_set | set(items)
    max_item = max(item_set)
    ...
    valid_rating_matrix = generate_rating_matrix_valid(user_seq, num_users, num_items)
    test_rating_matrix = generate_rating_matrix_test(user_seq, num_users, num_items)
    return user_seq_attribute, user_seq, max_item, valid_rating_matrix, test_rating_matrix
...
def get_item2attribute_json(data_file, codebook_size):
    ...
    for item, attributes in item2attribute.items():
        ...
        attribute2item[tuple(attributes[:codebook_size])] = item
        if tuple(attributes[:codebook_size]) in count:
            count[tuple(attributes[:codebook_size])] = count[tuple(attributes[:codebook_size])] + 1
        else: count[tuple(attributes[:codebook_size])] = 0
    # import pdb
    # pdb.set_trace()
    attribute_size = max(attribute_set) # 331
    return attribute2item, item2attribute, attribute_size
